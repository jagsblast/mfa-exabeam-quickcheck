...
def finder(file_path):
    ...
    # Dictionary to track device changes by unique identifier
    device_unique_map = defaultdict(set)
    ...
    for name, token, version in device_data:
        ...
        # Use a unique identifier (name + version) to distinguish devices with the same name
        unique_device_id = f"{name}_{version}"
        device_unique_map[unique_device_id].add(token)

    # Find tokens with exactly 1 occurrence
    tokens_with_one_occurrence = [token for token, count in token_count.items() if count == 1]

    # Alert for devices with token changes, using unique identifiers to prevent false positives
    print(colored("Alert: Devices with token changes:", 'red'))
    token_change_alerts = []

    for unique_device_id, tokens in device_unique_map.items():
        if len(tokens) > 1:
            ...
            device_name, app_version = unique_device_id.split('_')
            token_change_alerts.append(f"DeviceName: {device_name}, App Version: {app_version} has had a token change {changes_message}")
